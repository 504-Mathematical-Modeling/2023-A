...
% ---- 第 3 段：问题二（等尺寸优化设计）----
针对问题二，将"额定功率 $60$ MW 下单位面积功率最大"等价变形为"镜面总面积最小"，构建六角晶格生成、快速效率地图、贪心选镜、差分进化搜索与完整复核的五层框架：外层在塔位、镜面尺寸、安装高度与晶格参数共 $8$ 维空间搜索，内层按单镜年均贡献排序选取最少镜面，复核层以 $60$ 时刻完整追迹验证。优化得塔位$(3.5,\,-61.0)$ m、镜面$6.43\times5.91$ m$^2$、安装高度$2.95$ m、定日镜$3112$面、镜面总面积$118120$ m$^2$，年平均输出热功率$60.39$ MW、单位面积功率$0.5113$ kW/m$^2$。与问题一相比，额定输出功率提高约$71\%$，而单位面积功率略有下降，这是镜场布局更密集、阴影遮挡效率降低（$0.821$ 对 $0.928$）的必然结果，属功率与密度的权衡。

% ---- 第 4 段：问题三（异尺寸优化设计）----
针对问题三，按距塔距离将镜场划分为三个环形带，带内统一尺寸与安装高度，外层差分进化在塔位与各带尺寸共 $11$ 维空间搜索（并以问题二最优解注入初始种群），内层按单镜年均单位面积贡献贪心选镜，另设局部精调层对安装高度做逐带坐标轮换档位扫描，并对低贡献边际镜做删除优化。得塔位$(3.5,\,-61.0)$ m、定日镜$3043$面、镜面总面积$115501.3$ m$^2$，年平均输出热功率$60.01$ MW、单位面积功率$0.5196$ kW/m$^2$。三带最优尺寸收敛于问题二同一组参数，而安装高度呈"近低远高"阶梯（近塔两带$2.95$ m、远塔带$6.0$ m），径向分层与边际镜删除使阴影遮挡效率提升至$0.8364$，单位面积功率较问题二提高约$1.6\%$，说明问题三的增益主要来自安装高度的径向分层与镜数精化而非尺寸异质化。
...
六角晶格是候选布局的生成策略而非题目额外约束：外层搜索其旋转角与相位，内层在格点上贪心选镜，最终高精度复核阶段通过补镜/删镜做局部改进，改进后仍满足全部约束的点才能保留。
...
$\eta_{\mathrm{cos}}$ 与 $\eta_{\mathrm{at}}$ 可解析向量化计算，$\eta_{\mathrm{trunc}}$ 采用问题一的光锥-圆柱求交；评价阶段使用低精度追迹（镜面采样 $6\times6$、光锥光线 $32$ 根），并按设计参数 $(T,w,H,a)$ 对同一外层个体的全部候选镜复用。候选点仍同时检查场地圆与塔周禁装区，场外点直接判为无效。

快速无阴影功率估计为
\begin{equation}
    P_{\mathrm{fast}} = \eta_{\mathrm{ref}} \, \sum_{i} w H \, \Phi_i
    \label{eq:q2_pfast}
\end{equation}
该估计不含阴影遮挡与截断损失，仅用于排序与筛选，最终功率一律用完整模型重算；阴影遮挡与截断损失通过内层选镜目标的经验折扣（见下节 $0.75$）近似计入。

\subsubsection{内层贪心选镜}

对每个候选布局：
\begin{enumerate}
    \item 计算单镜解析年均单位面积贡献 $c_i = \overline{DNI \cdot \eta_{\mathrm{cos}} \cdot \eta_{\mathrm{at}}}$（kW/m$^{2}$，60 个代表时刻均值，不含阴影与截断近似），按降序排列；
    \item 累加 $c_i w H$ 直到目标 $P_{\mathrm{target}} = 60 / (0.92 \times 0.75)\;\mathrm{MW}$，其中 $0.75$ 为对阴影遮挡与截断损失的经验折扣，得到初始镜面数 $N_0$；
    \item 高精度复核：功率不足时按解析贡献序列依次补入后续镜面，功率超标过多时删除贡献最低的镜面。
\end{enumerate}

由于同一设计的单镜面积相同，镜面数 $N$ 最小即总面积最小，内层无需把 $N$ 作为独立连续变量搜索，离散调整由补镜/删镜操作完成。
...
采用差分进化（DE）做全局搜索，种群规模 $64$（$8$ 倍决策维数）、迭代 $20$ 代；每个个体只调用快速效率地图与内层选镜，不调用高精度追迹。适应度采用带功率缺口惩罚的面积目标：
\begin{equation}
    F(\mathbf{x}) = A_{\mathrm{total}} + \lambda \cdot \max\left(0,\, 61.2 - P_{\mathrm{fast}}\right)^2
    \label{eq:q2_fitness}
\end{equation}
其中 $\lambda = 10^{4}$ 为常数，其量级远大于单镜面积变化，保证优化优先满足额定功率约束；对功率缺口较大的个体，惩罚项主导适应度从而被迅速淘汰。目标功率取 $61.2$ MW 而非 $60$ MW，是为抵消低精度评价的系统性高估（约 $0.5\%$），使最终高精度口径下以不小于 $60$ MW 的概率显著提升。

对外层排名前 $3$ 的候选执行中精度复核与局部改进：
\begin{itemize}
    \item 功率不足：按解析贡献序列依次补入后续镜面；
    \item 功率超标过多：删除贡献最低的镜面；
    \item 每次改动后用中精度（$8\times8$ 网格、$512$ 光线）的 60 时刻追迹重新校验，最多调整 8 次。
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        阶段 & 镜面网格 & 光锥光线数 & 用途 \\
        \midrule
        外层快速评价 & $6\times6$ & $32$ & DE 适应度 \\
        候选复筛 & $8\times8$ & $512$ & 排名前 3 复核与补删镜 \\
...
优化得到的设计方案如下：
\begin{itemize}
    \item 吸收塔位置：$(x_T,\,y_T) = (3.5,\,-61.0)$ m；
    \item 镜面尺寸：$w \times H = 6.43 \times 5.91$ m$^{2}$，安装高度 $a = 2.95$ m；
    \item 定日镜数 $N = 3112$ 面，镜面总面积 $A_{\mathrm{total}} = 118120.3$ m$^{2}$；
    \item 年平均输出热功率 $\bar P = 60.39$ MW，单位镜面面积年平均输出热功率 $q = 0.5113$ kW/m$^{2}$；
    \item 年均光学效率 $\eta = 0.5252$（余弦 $0.7882$、阴影遮挡 $0.8213$、截断 $0.9359$）。
\end{itemize}

优化布局如图~\ref{fig:q2_layout} 所示，塔位南移约 $61$ m，镜面以六角晶格布满半径 $350$ m 场地圆与半径 $100$ m 禁装区之间的环形区域，北密南疏。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/fig_q2_layout.png}
    \caption{问题二优化布局（六角晶格，$3112$ 面定日镜）}
    \label{fig:q2_layout}
\end{figure}
...
\subsubsection{环形带分区降维}

$5N$ 维的逐镜优化规模过大，直接求解不可行。注意到镜场以吸收塔为中心呈环形结构，镜面到塔的距离是影响光学性能的首要空间因素，本文按距塔距离把场地划分为 $K=3$ 个环形带（以候选镜到塔距离的 $33\%$、$66\%$ 分位为界），带内统一尺寸与安装高度：
\begin{equation}
    (w,\,H,\,a) = (w_k,\,H_k,\,a_k),\qquad p \in \mathcal{R}_k,\ k = 1,2,3
    \label{eq:q3_bands}
\end{equation}
各带独立生成六角晶格候选点（晶格间距 $d_k = w_k + 5\;\mathrm{m}$），带内天然满足式~\eqref{eq:q3_spacing}；跨带候选点之间的间距冲突通过邻接表（间距小于 $\max(w_i,w_j)+5$ 时互斥）显式排除。此时决策变量缩减为塔位与各带尺寸，共
\begin{equation}
    \underbrace{(x_T,\,y_T)}_{2} + \underbrace{3 \times (w_k,\,H_k,\,a_k)}_{9} = 11\;\text{维}.
    \label{eq:q3_dim}
\end{equation}
物理动机：近塔区域单镜贡献密度高但相互遮挡严重，宜用小镜密布；远塔区域光锥截断损失大，宜用大镜弥补功率。分区结构允许优化器在"近小远大"与"全局统一"之间权衡。

\subsubsection{双层求解框架}

沿用问题二的"外层全局搜索 + 内层选镜"框架：
\begin{enumerate}
    \item 外层：差分进化搜索 $11$ 维决策空间（塔位与三带尺寸），种群规模 $110$（$10$ 倍决策维数）、迭代 $40$ 代；晶格旋转角与相位沿用问题二最优解（其对年均功率的影响小于 $0.2\%$，见敏感性分析），并以问题二最优解作为初始种群注入（$x_0$ 种子），加快收敛；
    \item 内层：对合并后的全部候选镜按解析年均单位面积贡献 $\Phi_i / (w_i H_i)$ 降序贪心选镜，累计功率达到解析目标
    \begin{equation}
        P_{\mathrm{tar}} = \frac{60 \times 1.02}{\eta_{\mathrm{ref}} \times 0.75}\;\mathrm{MW} \approx 88.7\;\mathrm{MW}
        \label{eq:q3_target}
    \end{equation}
    即停止（系数 $0.92 \times 0.75$ 为反射率与阴影遮挡/截断的解析折扣，$1.02$ 为低精度高估补偿），选镜过程跳过冲突表中的互斥点；
    \item 复核层：与问题二相同，对排名前 $3$ 的候选做 $8\times8/512$ 高精度复算并补镜/删镜局部调整，最终方案以 $10\times10/1024$ 精度验收（不足 $60\;\mathrm{MW}$ 判不合格）；
    \item 局部精调层：安装高度的径向分层收益（约 $1\%$）与低精度评价偏差同量级，全局搜索难以分辨，故对选定方案固定塔位、镜位与镜面尺寸，逐带在安装高度档位 $a \in \{2.0,\, 2.5,\, \dots,\, 6.0\}$ 上做坐标轮换：每次只改变一个带的安装高度并评估（低精度 $6\times6/32$，各档位并行），接受使功率提升的档位，循环直至整轮无改进；随后进行边际镜删除优化：在完整追迹（低精度）下统计每面镜的年均功率贡献，逐轮删除贡献最低的镜面并重新追迹更新贡献，直至平均功率降至额定值附近，删除方案以 $8\times8/512$ 与 $10\times10/1024$ 两级精度复核，任一精度下不足额定功率即回退维持原方案。
\end{enumerate}
...
\begin{itemize}
    \item 吸收塔位置：$(x_T,\,y_T) = (3.5,\,-61.0)$ m；
    \item 定日镜数 $N = 3043$ 面，镜面总面积 $A_{\mathrm{total}} = 115501.3$ m$^{2}$；
    \item 各带尺寸均为 $w \times H = 6.43 \times 5.91$ m$^{2}$；安装高度呈"近低远高"阶梯：近塔两带 $a = 2.95$ m（$= H/2$ 触地下界）、远塔带 $a = 6.0$ m（上限）（见结果文件 \texttt{result3.xlsx}）；
    \item 年平均输出热功率 $\bar P = 60.01$ MW，单位镜面面积年平均输出热功率 $q = 0.5196$ kW/m$^{2}$；
    \item 年均光学效率 $\eta = 0.5338$（余弦 $0.7848$、阴影遮挡 $0.8364$、截断 $0.9361$）。
\end{itemize}

\subsubsection{结果分析}

三带最优尺寸收敛于同一组参数（$6.43 \times 5.91$ m$^{2}$），但安装高度呈现明显的"近低远高"阶梯结构：近塔两带取触地下界 $a = H/2 = 2.95$ m，远塔带（$r > 291.9$ m）抬升至上限 $a = 6.0$ m。这一结构源于几何差异：远塔镜反射光线的仰角小、镜面接近竖直，将镜面整体抬升可使其移出近旁镜在低太阳高度角时投下的遮挡阴影；近塔镜反射仰角大、镜面接近水平，保持较低安装高度可减小其对邻镜的遮挡。坐标轮换局部精调（见 5.3 节）自动确定该阶梯：与全场 $a = 2.95$ m 的基准相比，精调后阴影遮挡效率由 $0.8213$ 提升至 $0.8288$，平均功率由 $60.57$ MW 提高至 $61.15$ MW（$+0.96\%$）；而全场统一抬高反而使近塔平躺镜的遮挡加剧、功率下降约 $1\%$。由于约 $1\%$ 的径向分层收益与低精度评价偏差同量级，外层差分进化未能自动发现该改进，局部精调层以档位扫描的方式在更高精度口径下将其捕捉，体现了"全局粗搜 + 局部精调"两级结构对小增益结构的互补性。

在局部精调的基础上，按单镜年均功率对各镜排序并逐轮删除低贡献的边际镜：删除过程中每轮重新追迹以更新各镜贡献，当平均功率降至额定附近时停止，共删除 $81$ 面年均贡献最低的镜（$N = 3043$，$A_{\mathrm{total}} = 115501.3$ m$^{2}$）。删除后阴影遮挡效率进一步提升至 $0.8364$（移除的镜多为自身贡献低且遮挡邻镜的边际镜），平均功率 $60.01$ MW 在额定功率之上，单位面积功率由 $0.5157$ 提高至 $0.5196$ kW/m$^{2}$（$+0.76\%$）。严格口径（$10\times10$ 网格、$1024$ 光线）下最终功率恰好越过 $60$ MW 额定线，说明 $61.15$ MW 的解中约有 $1.1$ MW 的功率由低效边际镜贡献，删除这类镜可在不牺牲额定达标的前提下提升镜场整体经济性。

相比之下，镜面尺寸对单镜贡献的影响主要体现在面积与间距上，单位面积贡献随尺寸的变化率在 $w \approx 6.4\;\mathrm{m}$ 附近很小，故"近小远大"的尺寸分区并未带来可观测增益。综合而言，问题三的增益主要来自安装高度的径向分层与边际镜删除，而非尺寸异质化。
