...
\small
\begin{longtable}{c@{\hspace{7.5em}}c@{\hspace{7.5em}}l}
    \toprule
    符号 & 意义说明 & 单位 \\
    \midrule
    \endfirsthead
    ...
    \endlastfoot
    $H_i$ & 第 $i$ 面定日镜的镜心坐标 & m \\
    $\mathbf{n}_i$ & 第 $i$ 面定日镜的法向量 & — \\
    $\mathbf{s}$ & 太阳方向单位向量 & — \\
    $\mathbf{r}_i$ & 第 $i$ 面定日镜的反射方向单位向量 & — \\
    $\varphi$ & 当地地理纬度 & rad \\
    $\delta$ & 太阳赤纬角 & rad \\
    $\omega$ & 太阳时角 & rad \\
    $\alpha_s$ & 太阳高度角 & rad \\
    $D$ & 以春分（3 月 21 日）为第 0 天的天数 & 天 \\
    $G_0$ & 太阳常数，$G_0 = 1.366$ & kW/m$^{2}$ \\
    $\eta_{\mathrm{cos}}$ & 余弦效率 & — \\
    $\eta_{\mathrm{sb}}$ & 阴影遮挡效率 & — \\
    $\eta_{\mathrm{at}}$ & 大气透射率 & — \\
    $\eta_{\mathrm{trunc}}$ & 截断效率 & — \\
    $\eta_{\mathrm{ref}}$ & 镜面反射率 & — \\
    $\eta$ & 综合光学效率 & — \\
    $W$ & 定日镜边长（问题一中 $W = 6$） & m \\
    $h$ & 定日镜安装高度（问题一中 $h = 4$） & m \\
    $R$ & 集热器半径，$R = 3.5$ & m \\
    $A_i$ & 第 $i$ 面定日镜的镜面面积 & m$^{2}$ \\
    $N$ & 定日镜总数，$N = 1745$ & 面 \\
    $\sigma$ & 太阳光锥半角，$\sigma = 4.65$ & mrad \\
    $d_{\mathrm{HR}}$ & 镜面中心到集热器中心的空间距离 & m \\
    $R_c$ & 候选遮挡镜面筛选半径 & m \\
    DNI & 法向直射辐照度 & kW/m$^{2}$ \\
    $P$ & 镜场输出热功率 & MW \\
    $q$ & 单位镜面面积输出热功率 & kW/m$^{2}$ \\
\end{longtable}
...
求解流程分为五步：\textbf{首先}由日期与时刻计算太阳赤纬角、时角与太阳方向向量；\textbf{其次}由反射定律确定每面镜的法向量，解析计算余弦效率与大气透射率；\textbf{再次}在每面镜面上布设采样网格，通过射线-矩形求交判定塔影、镜间阴影与挡光，得到阴影遮挡效率；\textbf{然后}用 Sobol 序列在太阳光锥内采样，通过射线-圆柱求交判定命中率，得到截断效率；\textbf{最后}将五个分量相乘得到综合光学效率，并按 $60$ 个代表时刻汇总为月均与年均指标。下面依次建立各分量的计算模型。
...
\end{equation}
其中 $\bar{\eta}(m,k)$ 为第 $m$ 月 21 日第 $k$ 个代表时刻的场平均光学效率。输出热功率与单位面积输出热功率的月平均、年平均均按同样口径计算：
\begin{equation}
